...
bool getShoulderLength(const openpose_ros_msgs::OpenPoseHuman &person, double& length){
  ...
  for(int ix = 0; ix < person.body_key_points_with_prob.size(); ix++){
    openpose_ros_msgs::PointWithProb body_part = person.body_key_points_with_prob[ix];
    // Neck
    if(ix == 1){
...
void calcJointStates(const openpose_ros_msgs::OpenPoseHuman &person_to_follow){
  ROS_INFO("calcJointStates");

  joint_angles_.clear();

  for(int ix = 0; ix < person_to_follow.body_key_points_with_prob.size(); ix++){
    openpose_ros_msgs::PointWithProb body_part = person_to_follow.body_key_points_with_prob[ix];
    body_position_[ix] = Eigen::Vector2d(body_part.x, body_part.y);
  }
...
void publishJointStates(){
  ...
  if(left_pos >= 0 && left_pos <= M_PI)
    left_pos = M_PI - left_pos;
  else
    left_pos = -M_PI -left_pos;

  if(right_pos >= 0 && right_pos <= M_PI)
  ...
  motorSetPositionSrv_1.request.value = left_pos;
  motorSetPositionSrv_2.request.value = right_pos;
...
void humanPoseCallback(const openpose_ros_msgs::OpenPoseHumanList::ConstPtr &msg) {
...
int main(int argc, char **argv) {
  ...
  // subscribe openpose
  ros::Subscriber human_pose_sub_ = n.subscribe("/openpose_ros/human_list", 1, humanPoseCallback);

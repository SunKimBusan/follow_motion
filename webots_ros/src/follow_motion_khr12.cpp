...
std::map<std::string, double> prev_joint_angles_;
...
const double body_parts_lim_scale = 1.15;
const double l_upper_arm_len_lim = 0.45*body_parts_lim_scale;
const double l_lower_arm_len_lim = 0.45*body_parts_lim_scale;
const double r_upper_arm_len_lim = 0.45*body_parts_lim_scale;
const double r_lower_arm_len_lim = 0.45*body_parts_lim_scale;
const double shoulder_len_lim = 0.6*body_parts_lim_scale;
const double back_len_lim = 0.6*body_parts_lim_scale;

// When shoulder and elbow overlap in the image
const double upper_arm_len_min = 0.15;
const double lower_arm_len_min = 0.15;
...
void calcJointStates(const openpose_ros_msgs::OpenPoseHuman3D &person_to_follow){
  ...
  bool is_new_left = false;
  bool is_new_right = false;
  ...
  if(body_position_[LShoulder] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    body_position_[Neck] != Eigen::Vector3d(0.0, 0.0, 0.0)){

    // filter abnormal body part length
    double l_upper_arm_len = (body_position_[LElbow] - body_position_[LShoulder]).norm();
    double l_lower_arm_len = (body_position_[LWrist] - body_position_[LElbow]).norm();
    double shoulder_len = (body_position_[LShoulder] - body_position_[RShoulder]).norm();
    double back_len = (body_position_[Neck] - body_position_[MidHip]).norm();

    ROS_INFO("l_upper_arm_len : %f", l_upper_arm_len);
    ROS_INFO("l_lower_arm_len : %f", l_lower_arm_len);
    ROS_INFO("shoulder_len : %f", shoulder_len);
    ROS_INFO("back_len : %f", back_len);


    if(l_upper_arm_len < l_upper_arm_len_lim &&
      l_lower_arm_len < l_lower_arm_len_lim &&
      shoulder_len < shoulder_len_lim &&
      back_len < back_len_lim &&
      l_upper_arm_len > upper_arm_len_min &&
      l_lower_arm_len > lower_arm_len_min){
      ...
      prev_joint_angles_["l_sho_pitch"] = theta0;
      prev_joint_angles_["l_sho_roll"] = theta1;
      prev_joint_angles_["l_el_pitch"] = theta2;
      prev_joint_angles_["l_el"] = theta3;
      is_new_left = true;
  ...
  if(body_position_[RShoulder] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    // filter abnormal body part length
    double r_upper_arm_len = (body_position_[RElbow] - body_position_[RShoulder]).norm();
    double r_lower_arm_len = (body_position_[RWrist] - body_position_[RElbow]).norm();
    double shoulder_len = (body_position_[LShoulder] - body_position_[RShoulder]).norm();
    double back_len = (body_position_[Neck] - body_position_[MidHip]).norm();

    if(r_upper_arm_len < r_upper_arm_len_lim &&
      r_lower_arm_len < r_lower_arm_len_lim &&
      shoulder_len < shoulder_len_lim &&
      back_len < back_len_lim &&
      r_upper_arm_len > upper_arm_len_min &&
      r_lower_arm_len > lower_arm_len_min){
      ...
      prev_joint_angles_["r_sho_pitch"] = theta0;
      prev_joint_angles_["r_sho_roll"] = theta1;
      prev_joint_angles_["r_el_pitch"] = theta2;
      prev_joint_angles_["r_el"] = theta3;
      is_new_right = true;

    }
  }

  if (!is_new_left && is_new_right){
    joint_angles_["l_sho_pitch"] = prev_joint_angles_["l_sho_pitch"];
    joint_angles_["l_sho_roll"] = prev_joint_angles_["l_sho_roll"];
    joint_angles_["l_el_pitch"] = prev_joint_angles_["l_el_pitch"];
    joint_angles_["l_el"] = prev_joint_angles_["l_el"];
  } else if(is_new_left && !is_new_right){
    joint_angles_["r_sho_pitch"] = prev_joint_angles_["r_sho_pitch"];
    joint_angles_["r_sho_roll"] = prev_joint_angles_["r_sho_roll"];
    joint_angles_["r_el_pitch"] = prev_joint_angles_["r_el_pitch"];
    joint_angles_["r_el"] = prev_joint_angles_["r_el"];
  }
}

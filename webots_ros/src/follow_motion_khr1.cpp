...
void calcJointStates(const openpose_ros_msgs::OpenPoseHuman3D &person_to_follow){
  ...
  if(body_position_[Neck] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    checkMaxAngle(90 * M_PI / 180.0, l_shoulder_roll);

    // change joint angle to each webots joint standard
    l_shoulder_roll = M_PI/2 - l_shoulder_roll;
  ...
  if(body_position_[Neck] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    checkMaxAngle(90 * M_PI / 180.0, r_shoulder_roll);

    // change joint angle to each webots joint standard
    r_shoulder_roll = M_PI/2 - r_shoulder_roll;
  ...
  if(body_position_[Neck] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    checkMaxAngle(2.36, l_shoulder_pitch);
  ...
  if(body_position_[Neck] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    checkMaxAngle(2.36, r_shoulder_pitch);

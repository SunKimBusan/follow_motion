...
void publishJointStates(){
  ...
  for (int i=1; i<=8; ++i){
    dxl_wb.readRegister(i, "Present_Position", &present_pos[i-1], &logg);
  }
  ...
  for (int i=1; i<=8; ++i){
    ...
    dxl_wb.writeRegister(i, "Profile_Velocity", p_vel, &logg);
    dxl_wb.writeRegister(i, "Profile_Acceleration", p_acc, &logg);
  }

  for (int i=1; i<=8; ++i){
    dxl_wb.writeRegister(i, "Goal_Position", goal_pos[i-1], &logg);
  }

...
void calcJointStates(const openpose_ros_msgs::OpenPoseHuman3D &person_to_follow){
  ...
  if(body_position_[LShoulder] != Eigen::Vector3d(0.0, 0.0, 0.0) &&
    ...
    body_position_[Nose] != Eigen::Vector3d(0.0, 0.0, 0.0)){
    ...
    Eigen::Vector3d d_vector = body_position_[Nose];

    Eigen::Vector3d b_minus_a_vector = b_vector - a_vector;
    Eigen::Vector3d z_basis = b_minus_a_vector/b_minus_a_vector.norm();

    double alpha = pow(b_vector.coeff(0)-a_vector.coeff(0),2)+pow(b_vector.coeff(1)-a_vector.coeff(1),2)+pow(b_vector.coeff(2)-a_vector.coeff(2),2);
    double beta = (b_vector.coeff(0)-a_vector.coeff(0))*(a_vector.coeff(0)-c_vector.coeff(0))
    ...
    double gamma1 = d_vector.coeff(0)*n_vector.coeff(1)-d_vector.coeff(1)*n_vector.coeff(0);
    double beta1 = d_vector.coeff(0)*n_vector.coeff(2)-d_vector.coeff(2)*n_vector.coeff(0);
    double delta = -n_vector.coeff(0)*c_vector.coeff(0)-n_vector.coeff(1)*c_vector.coeff(1)-n_vector.coeff(2)*c_vector.coeff(2);
    double alpha2 = n_vector.coeff(0)+(pow(n_vector.coeff(1),2)+pow(n_vector.coeff(2),2))/n_vector.coeff(0);
    double beta2 = delta-(n_vector.coeff(1)*gamma1+n_vector.coeff(2)*beta1)/n_vector.coeff(0);
    double dp_x = -beta2/alpha2;
    double dp_y = n_vector.coeff(1)/n_vector.coeff(0)*dp_x-gamma1/n_vector.coeff(0);
    double dp_z = n_vector.coeff(2)/n_vector.coeff(0)*dp_x-beta1/n_vector.coeff(0);

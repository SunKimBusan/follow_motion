...
#include "turtlebot3_ttl_mecanum_core_config.h"
...
void loop()
{
  ...
  if ((t-tTime[0]) >= (1000 / CONTROL_MOTOR_SPEED_FREQUENCY))
  {
    ...
    if ((t-tTime[6]) > CONTROL_MOTOR_TIMEOUT)
    {
      ...
      motor_driver.controlMotor(WHEEL_RADIUS, WHEEL_SEPARATION, goal_velocity);           /////////////goal_velocity를 받아와서 모터를 움직여야됨
    }
    tTime[0] = t;
  }
...
/*******************************************************************************
* Publish msgs (CMD Velocity data from RC100 : angular velocity, linear velocity)       //수정필요 y추가 필요 아니 애초에 별로 필요 없어보임
*******************************************************************************/
...
//  dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder); ////수정필요 .인코더 //모터드라이버는 수정 되어있음
  // dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.left_rear_encoder, sensor_state_msg.right_rear_encoder, sensor_state_msg.left_front_encoder, sensor_state_msg.right_front_encoder); ////수정완료
...
//    updateMotorInfo(sensor_state_msg.left_encoder, sensor_state_msg.right_encoder);   /////수정필요 .ㅗ
    // updateMotorInfo(sensor_state_msg.left_rear_encoder, sensor_state_msg.right_rear_encoder, sensor_state_msg.left_front_encoder, sensor_state_msg.right_front_encoder);   /////수정완료
...
//  odom.twist.twist.linear.x  = odom_vel[0];         ////수정필요 linear y 추가
...
*******************************************************************************/
//void updateMotorInfo(int32_t left_tick, int32_t right_tick)                       ////수정필요    rear front만 추가해주면 되는데 나중에 전반적으로 고쳐줘야됨
...
/*******************************************************************************
* Turtlebot3 test drive using push buttons                                          ////수정필요
*******************************************************************************/

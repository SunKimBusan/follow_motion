...
#define JOINT_NUM                   10  //4 wheels, 4 arm joints, 2 gripper joints
...
// 이부분 수정되어야 할 것 같음.
void armCallback(const trajectory_msgs::JointTrajectory& arm_msg);
...
//void updateMotorInfo(int32_t left_tick, int32_t right_tick);  ////수정필요
...
// 이부분 체크해봐야 함. arm_pos 어떻게 구성되어 있는지.
ros::Subscriber<trajectory_msgs::JointTrajectory> arm_sub("arm_pos", armCallback);
...
*******************************************************************************/
// 아래 클래스 어떻게 구성되어 있는지 확인해야함.
Turtlebot3MotorDriver motor_driver;

char debug_buf[255];
// 아래인자는 아마 안쓰지 싶어요.
bool gripper_update=false;
...
// 아래 인자들 다 안쓰거나 변형해야할 것 같다.
float arm_target_position[4]={0.0,0.0,   3.141592/180.0*85, -3.141592/2.0};
float arm_current_position[4]={0.0,0.0,   3.141592/180.0*85, -3.141592/2.0};
float gripper_current_position=0.0;
bool arm_inited;
// 그리퍼 안씀 아랫줄 주석처리해야 할 듯.

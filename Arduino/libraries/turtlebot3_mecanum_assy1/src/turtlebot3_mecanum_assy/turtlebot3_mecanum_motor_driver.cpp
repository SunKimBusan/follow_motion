...
bool Turtlebot3MotorDriver::init(String turtlebot3, int baudrate, char* debug_buf){
  ...
  // 아래부분 수정해야하지 싶다.
  // robot arm, gripper init 하는데 쓰이는 객체고. workbench 스타일로 변경
  // 다시 보니 그냥 로봇암, 그리퍼 모터 위치 읽어주는데 필요한 객체다.
  ...
  // robot arm control 하는데 쓰이는 객체, workbench 스타일로 변경
  ...
  // Control Gripper 에 쓰이는 부분인데 제거하면 될 듯.
...
bool Turtlebot3MotorDriver::setTorque(bool onoff){
  ...
  // 아래부분 workbench style 로 토크 걸어줘야함.
...
// 이부분 없애든지 수정하던지.
// 이부분이 로봇암 read 부분 같은데... 없애든지 workbench style로 변경을 해야하지 싶다.
